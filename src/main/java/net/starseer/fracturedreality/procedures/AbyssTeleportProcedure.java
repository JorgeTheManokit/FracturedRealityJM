...
@EventBusSubscriber
public class AbyssTeleportProcedure {
	@SubscribeEvent
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double y, Entity entity) {
		if (entity == null)
			return;
		if (FracturedRealityModVariables.MapVariables.get(world).AbyssAccessible) {
			if ((entity.level().dimension()) == Level.OVERWORLD) {
				if (y <= -128) {
					if (entity instanceof ServerPlayer _player && !_player.level().isClientSide()) {
						ResourceKey<Level> destinationType = ResourceKey.create(Registries.DIMENSION, ResourceLocation.parse("fractured_reality:abyss"));
